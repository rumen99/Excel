...
Formula::Formula(std::string _equation)
{
    equation = _equation;
    isCalculated = false;
...
int Formula::get_size()
{
    ...
    if (res != std::nullopt)
        return (compress(std::to_string(res.value())).size());
    return 5;
}

void Formula::print(int sizeCell)
{
    std::optional res = get_value();
    ...
    if (res != std::nullopt)
    {
        ...
        for (int i = get_size(); i < sizeCell; ++i)
            std::cout << " ";
        return;
    }
    else
